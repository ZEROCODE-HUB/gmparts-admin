...
export default function BoletaCompraList() {
  const navigate = useNavigate();
  const [items, { remove }] = useFirestoreDocuments("c-boleta");
  ...
  return (
    <div>
      <Toolbar title="Compra - Boleta" count={rows.length} onNew={() => navigate("/c-boleta/nuevo")} onExport={() => exportToExcel(rows, "BoletasCompra")} />
      ...
      <Table columns={["Serie", "Número", "Fecha", "Proveedor", "Documento", "Total", "Estado", "Acción"]}
        ...
        renderRow={(c) => (
          <>
            ...
            <Td><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700`}>{c.estado || "?"}</span></Td>
            <Td>
              <div className="flex gap-1">
                ...
                <EnviarSunatButton docKey="c-boleta" id={c.id} estadoActual={c.estadoSunat || c.estadoFactura} esPrueba={c.sunatEsPrueba} reintentable={c.sunatReintentable} correoEnviado={c.correoEnviadoA} />
      ...
      {deleteTarget && (
        <Modal title="Anular boleta" onClose={() => setDeleteTarget(null)}>
          <p className="text-sm text-[var(--muted)] mb-6">¿Estás seguro de anular esta boleta?</p>
