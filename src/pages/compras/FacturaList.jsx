...
export default function FacturaCompraList() {
  const navigate = useNavigate();
  const [items, { remove }] = useFirestoreDocuments("c-factura");
  ...
  return (
    <div>
      <Toolbar title="Compra - Factura" count={rows.length} onNew={() => navigate("/c-factura/nuevo")} onExport={() => exportToExcel(rows, "FacturasCompra")} />
      ...
      <Table columns={["Serie", "Número", "Fecha", "Proveedor", "Documento", "Total", "Estado", "Acción"]}
        ...
        renderRow={(c) => (
          <>
            ...
            <Td><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${c.estado === "Creado" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>{c.estado || "?"}</span></Td>
            <Td>
              <div className="flex gap-1">
                ...
                <button onClick={() => navigate(`/c-factura/${c.id}`)} className="p-1.5 rounded-md text-[var(--accent)] hover:bg-[var(--accent-dim)]" title="Editar"><Pencil size={15} /></button>
                <EnviarSunatButton docKey="c-factura" id={c.id} estadoActual={c.estadoSunat || c.estadoFactura} esPrueba={c.sunatEsPrueba} reintentable={c.sunatReintentable} correoEnviado={c.correoEnviadoA} />
      ...
      {deleteTarget && (
        <Modal title="Anular factura" onClose={() => setDeleteTarget(null)}>
          <p className="text-sm text-[var(--muted)] mb-6">¿Estás seguro de anular esta factura?</p>
      ...
      {preview && <DocumentPreviewModal title="Vista previa - Factura Compra" data={preview} fields={previewFields} collection="FacturasVentasCompras" onClose={() => setPreview(null)} />}
