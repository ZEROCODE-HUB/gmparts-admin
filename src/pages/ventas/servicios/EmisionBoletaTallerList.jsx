...
export default function EmisionBoletaTallerList() {
  ...
  const [items] = useFirestoreDocuments("vs-boleta");
  ...
  return (
    <div>
      <Toolbar title="Emisi\u00f3n Boleta Taller" count={rows.length} onNew={() => navigate("/vs-boleta/nuevo")} onExport={() => exportToExcel(rows, "BoletasTaller")} />
      <SearchBox value={q} onChange={setQ} />
      <DateRangeFilter fechaDesde={fechaDesde} fechaHasta={fechaHasta} onChange={(d, h) => { setFechaDesde(d); setFechaHasta(h); }} />
      <Table columns={["Serie", "Número", "Fecha", "Cliente", "DNI", "Servicio", "Total", "Acción"]}
      ...
      {deleteTarget && (
        <AnularComprobanteModal docKey="vs-boleta" id={deleteTarget.id} onClose={() => setDeleteTarget(null)} />
      )}
