...
export default function RegistroNotaVentasList() {
  ...
  return (
    <div>
      ...
      <Table columns={["Serie", "N?mero", "Fecha", "Cliente", "Documento", "Servicio", "Total", "Acci?n"]}
        rows={rows}
        renderRow={(c) => (
          <>
            ...
            <Td>
              <div className="flex gap-1">
                ...
                <button onClick={() => alert("Canjear nota de venta: funcionalidad pendiente de implementar.")} className="p-1.5 rounded-md text-[var(--accent)] hover:bg-[var(--accent-dim)]" title="Canjear"><Repeat size={15} /></button>
      ...
      {deleteTarget && (
        <Modal title="Anular nota" onClose={() => setDeleteTarget(null)}>
          <p className="text-sm text-[var(--muted)] mb-6">?Est?s seguro de anular esta nota?</p>
