...
export default function NotaCreditoList() {
  ...
  return (
    <div>
      <Toolbar title="Nota de crédito" count={rows.length} onNew={() => navigate("/va-notacredito/nuevo")} onExport={() => exportToExcel(rows, "NotasCredito")} />
      ...
      <Table columns={["Serie", "Número", "Fecha", "Cliente", "Documento", "Motivo", "Total", "Acción"]}
          ...
          <>
            ...
            <Td>
              <div className="flex gap-1">
                ...
                <EnviarSunatButton docKey="va-notacredito" id={c.id} estadoActual={c.estadoSunat || c.estadoFactura} esPrueba={c.sunatEsPrueba} reintentable={c.sunatReintentable} correoEnviado={c.correoEnviadoA} />
      ...
      />
      {deleteTarget && (
        <Modal title="Anular nota de crédito" onClose={() => setDeleteTarget(null)}>
          <p className="text-sm text-[var(--muted)] mb-6">¿Estás seguro de anular esta nota de crédito?</p>
      ...
      {preview && <DocumentPreviewModal title="Vista previa - Nota de Crédito" data={preview} fields={previewFields} collection="FacturasVentasCompras" onClose={() => setPreview(null)} />}
