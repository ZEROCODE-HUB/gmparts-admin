...
const previewFields = [
  ...
  { key: "clienteDoc", label: "RUC" }, { key: "direccion", label: "Dirección" },
  { key: "total", label: "Total" }, { key: "estado", label: "Estado" },
];

export default function GuiaRemisionList() {
  const navigate = useNavigate();
  const [items, { remove }] = useFirestoreDocuments("va-guia");
  ...
  return (
    <div>
      <Toolbar title="Guía de remisión" count={rows.length} onNew={() => navigate("/va-guia/nuevo")} onExport={() => exportToExcel(rows, "Guias")} />
      ...
      <Table columns={["Serie", "Número", "Fecha", "Cliente", "RUC", "Dirección", "Total", "Acción"]}
        sortable={[{key:"serie",label:"Serie"},{key:"numero",label:"N\u00famero"},{key:"fecha",label:"Fecha"},{key:"cliente",label:"Cliente"},{key:"total",label:"Total"}]}
        ...
        renderRow={(c) => (
          <>
            ...
            <Td className="text-[var(--muted)] max-w-[200px] truncate">{c.direccion || ""}</Td>
            ...
            <Td>
              <div className="flex gap-1">
                ...
                <button onClick={() => navigate(`/va-guia/${c.id}`)} className="p-1.5 rounded-md text-[var(--accent)] hover:bg-[var(--accent-dim)]" title="Editar"><Pencil size={15} /></button>
                <EnviarSunatButton docKey="va-guia" id={c.id} estadoActual={c.estadoSunat || c.estadoFactura} esPrueba={c.sunatEsPrueba} reintentable={c.sunatReintentable} correoEnviado={c.correoEnviadoA} />
      ...
      {deleteTarget && (
        <Modal title="Anular guía" onClose={() => setDeleteTarget(null)}>
          <p className="text-sm text-[var(--muted)] mb-6">¿Estás seguro de anular esta guía?</p>
      ...
      {preview && <DocumentPreviewModal title="Vista previa - Guía de Remisión" data={preview} fields={previewFields} collection="FacturasVentasCompras" onClose={() => setPreview(null)} />}
